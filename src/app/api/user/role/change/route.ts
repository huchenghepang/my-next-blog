...
import { change_role_Interface } from "@/types/params/user";
...
export const POST = createApiHandler(async (req) => {
    try {
        const { data, error } = await validateRequest<change_role_Interface>(req, change_role_schema);
        if (!data) {
        ...
        const isSuccess = await updateSessionData(sessionid, { user: { userId: data.userId, currentRole: { role_id: roles.role_id, role_name: roles.Roles.role_name } } })
        if (!isSuccess) return sendError({ errorMessage: "切换角色失败" });
        return sendResponse({ message: "切换角色成功", data: { currentRole: { role_id: roles.role_id, role_name: roles.Roles.role_name } } })
    } catch (error) {
