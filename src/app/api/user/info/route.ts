...
export const GET = createApiHandler(async () => {
  try {
    ...
    if (!session || !session.user)
      return sendError({errorMessage: "缺少验证信息", code: "403"})
    ...
    if (userId && currentRole && currentRole.role_id) {
      ...
      const newPermissions = permissions.map(rolePermission => {
        return rolePermission.Permissions
      })
      return sendResponse({
        data: {user: user, permissions: newPermissions, roles, currentRole},
      })
    }
