...
export const POST = createApiHandler(async (req) => {
    const data = await parseRequestDataAndQueryData<{ pagination?: PaginationParams, categoryId?: number }>(req);
    ...
    // 计算总页数
    const totalPages = Math.min(1, Math.ceil(totalCount / pageSize));
